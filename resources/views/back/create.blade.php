...
<body class="bg-gray-100 p-6 flex items-center justify-center min-h-screen">
    <div class="max-w-md w-full bg-white p-6 rounded-lg shadow-lg">
        <a href="{{ route('arm.index') }}" class="inline-block mb-4 text-white bg-purple-600 hover:bg-purple-700 focus:ring-4 focus:ring-purple-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-purple-500 dark:hover:bg-purple-600 dark:focus:ring-purple-800"> Back To the Main Table</a>

        <form method="POST" action="{{ route('arm.store') }}" enctype="multipart/form-data">
            @csrf
